...
public class AISteering : MonoBehaviour {
	...
	void Pursuit (){
        ...
        float iterationAhead = (target.transform.position - transform.position).magnitude / time;
       
		Vector3 targetSpeed = target.gameObject.GetComponent<Move>().instantVelocity;

		Vector3 targetFuturePosition = target.transform.position + (targetSpeed * iterationAhead * k);
	...
	void Evade (){
		...
		float iterationAhead = (transform.position - target.transform.position).magnitude / time;

		Vector3 targetSpeed = target.gameObject.GetComponent<Move>().instantVelocity;

		Vector3 targetFuturePosition = target.position + (targetSpeed * iterationAhead * k);
